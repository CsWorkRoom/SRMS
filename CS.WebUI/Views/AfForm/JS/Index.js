...
layui.use(["table", "jquery"], function () {
    ...
    table.on('tool(tablefilter)', function (obj) {
        ...
        if (layEvent === 'edit') {
            OpenTopWindow("编辑表单", 0, 0, "../AfForm/Edit?id=" + data.ID);
        } else if (layEvent === 'del') {
            layer.confirm('真的删除行么', function (index) {
                layer.close(index);
                var url = "../AfForm/Delete?id=" + data.ID;
...
//添加
function Add() {
    OpenTopWindow("添加表单", 0, 0, "../AfForm/Edit?id=0");
}
...
//导出
function ExportFile() {
    url = "../AfForm/ExportFile?name=" + $.trim($("#name").val());
    window.location = url;
}